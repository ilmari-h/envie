...
export default function EditProjectContent({ id: projectId }: { id: string }) {
  ...
  const confirmDeleteUser = () => {
    if (userToDelete) {
      removeUser({
        params: { id: projectId, userId: userToDelete.id },
      });
    ...
    <div>
      <main className="flex flex-col mt-8 gap-6 max-w-2xl mx-auto">
        <div className="flex items-center gap-3">
          <Link href={`/dashboard/project/${projectId}`} className="hover:text-neutral-400 transition-colors">
            <ArrowLeft size={16} />
          </Link>
        ...
        <ConfirmDialog
          ...
          onConfirm={() => {
            removeInviteLinks({ params: { id: projectId } });
